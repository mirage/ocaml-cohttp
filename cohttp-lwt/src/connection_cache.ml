...
let check_no_proxy_patterns host patterns =
  if String.length host = 0 || patterns = [] then false
  else
...
module Make_proxy (Connection : S.Connection) (Sleep : S.Sleep) = struct
  ...
  type t = {
    no_proxy : Connection_cache.t option;
    direct : Connection_cache.t option;
    tunnel : Connection_tunnel.t option;
    direct_proxies : (string * Connection_cache.t) list;
    tunnel_proxies : (string * Connection_tunnel.t) list;
    no_proxy_patterns : no_proxy_pattern list;
  }
  ...
  let create ?ctx ?keep ?retry ?parallel ?depth ?(scheme_proxy = []) ?all_proxy
    ...
    and create_tunnel proxy_uri =
      Connection_tunnel.create ?ctx ?keep ?retry ?parallel ?depth ?proxy_headers
        proxy_uri ()
    in
    match Option.map no_proxy_from_env no_proxy with
    | Some Wildcard ->
        {
          no_proxy = Some (create_default ());
          direct = None;
          tunnel = None;
          direct_proxies = [];
          tunnel_proxies = [];
    ...
    | no_proxy ->
        let direct_proxies, tunnel_proxies =
          List.partition_map
            (fun (scheme, uri) ->
              if List.mem scheme tunnel_schemes then
                Right (scheme, create_tunnel uri)
              else Left (scheme, create_direct uri))
            scheme_proxy
        in
        let direct, tunnel =
          match all_proxy with
          | Some uri -> (Some (create_direct uri), Some (create_tunnel uri))
          | None -> (Some (create_default ()), None)
        in
        let no_proxy, no_proxy_patterns =
          match no_proxy with
          | Some (Patterns t) -> (Some (create_default ()), t)
          | None -> (None, [])
          | _ -> assert false
        in
        {
  ...
  let call self ?headers ?body ?absolute_form meth uri =
    ...
    then
      let conn = Option.get self.no_proxy in
      Connection_cache.call conn ?headers ?body ?absolute_form meth uri
    else
      let scheme = Option.get (Uri.scheme uri) in
      let lookup list default =
        match List.assoc scheme list with
        | exception Not_found -> Option.get default
        | proxy -> proxy
      in
      if List.mem scheme tunnel_schemes then
        let proxy = lookup self.tunnel_proxies self.tunnel in
        Connection_tunnel.call proxy ?headers ?body ?absolute_form meth uri
      else
        let proxy = lookup self.direct_proxies self.direct in
